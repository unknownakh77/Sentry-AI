...
exports.getVpnApi = getVpnApi;
exports.getVirusTotalIp = getVirusTotalIp;
exports.getVirusTotalDomain = getVirusTotalDomain;
...
function setCache(key, data, ttlMs = 10 * 60 * 1000 /* 10 mins */) {
...
const FIXTURES = {
    ipinfo(ip) {
        if (ip === '185.220.101.1')
            return { ip, city: 'Moscow', region: 'Moscow', country: 'RU', loc: '55.7558,37.6173', org: 'AS197540' };
        if (ip === '203.0.113.45')
            return { ip, city: 'Unknown', region: 'Unknown', country: 'XX', loc: '0,0', org: 'AS99999' };
        return { ip, city: 'Toronto', region: 'Ontario', country: 'CA', loc: '43.6532,-79.3832', org: 'AS12345 Sentry ISP' }; // Default safe
    },
    vpnapi(ip) {
        if (ip === '185.220.101.1')
            return { ip, security: { vpn: true, proxy: false, tor: true, relay: false } };
        return { ip, security: { vpn: false, proxy: false, tor: false, relay: false } };
    },
    virustotalIp(ip) {
        if (ip === '185.220.101.1' || ip === '203.0.113.45')
            return { data: { attributes: { last_analysis_stats: { malicious: 12, suspicious: 4, harmless: 50 } } } };
        return { data: { attributes: { last_analysis_stats: { malicious: 0, suspicious: 0, harmless: 80 } } } };
    },
    vtDomain(domain) {
        if (domain === 'c0rp-support.com' || domain === 'bit.ly')
            return { data: { attributes: { last_analysis_stats: { malicious: 8, suspicious: 2, harmless: 40 } } } };
...
async function getVirusTotalDomain(domain) {
